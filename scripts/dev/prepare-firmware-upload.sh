...
# Spike S2 host overlay: project-vendored CoreS3 camera module that follows
# the official M5Stack/ESP-BSP path: shared I2C handle, esp_video DVP init,
# V4L2 frame dequeue, and JPEG conversion for the ECMA-419 camera API. Copied
# next to manifest_local.json so the host manifest can include it with a
# relative path.
...
prepared_host_overlay_dir="$target_dir/firmware/stackchan/tars-imagein-camera"

# The Moddable esp32/m5stack_cores3 target manifest is included after the root
# app manifest, so root-level build.SDKCONFIGPATH is overwritten by the target
# and camera Kconfig defaults never reach ESP-IDF. The host overlay manifest is
# included after the target; point it at a generated config directory that keeps
# the CoreS3 target defaults and appends the GC0308/esp_video camera options.
moddable_dir="${MODDABLE:-$HOME/.local/share/moddable}"
target_sdkconfig_dir="$moddable_dir/build/devices/esp32/targets/m5stack_cores3/sdkconfig"
combined_sdkconfig_dir="$prepared_host_overlay_dir/sdkconfig-combined"
rm -rf "$combined_sdkconfig_dir"
mkdir -p "$combined_sdkconfig_dir"
if [ -f "$target_sdkconfig_dir/sdkconfig.defaults" ]; then
  {
    echo "# Generated by scripts/dev/prepare-firmware-upload.sh"
    echo "# Base: $target_sdkconfig_dir/sdkconfig.defaults"
    cat "$target_sdkconfig_dir/sdkconfig.defaults"
    echo
    echo "# TARS CoreS3 camera overlay"
    cat "$prepared_host_overlay_dir/sdkconfig/sdkconfig.defaults"
  } >"$combined_sdkconfig_dir/sdkconfig.defaults"
else
  echo "warning: CoreS3 target sdkconfig defaults not found: $target_sdkconfig_dir/sdkconfig.defaults" >&2
  cp "$prepared_host_overlay_dir/sdkconfig/sdkconfig.defaults" "$combined_sdkconfig_dir/sdkconfig.defaults"
fi
if [ -f "$target_sdkconfig_dir/sdkconfig.defaults.release" ]; then
  cp "$target_sdkconfig_dir/sdkconfig.defaults.release" "$combined_sdkconfig_dir/sdkconfig.defaults.release"
fi
...
if (target === 'esp32/m5stack_cores3') {
  ...
  // module into the HOST firmware build. esp_video, esp32-camera conversion
  // helpers, and esp_jpeg are native IDF components, so they must be compiled
  // into the host (a runtime MOD cannot add native code) -- this is why
  // camera capture requires a host deploy flash, not `mod`-only. The
  // project overlay keeps the JS API shape but replaces the stock
  // esp_camera_init path with the official CoreS3 esp_video/V4L2 flow.
  ...
  // Keep the SDK target's GC0308 DVP pin map (sda 12 / scl 11 / d0..d7)
  // via manifest deep-merge, but force xclk=-1 to match the official CoreS3
  // BSP: the module uses the external 20 MHz camera clock and passes
  // xclk_freq to esp_video instead of driving GPIO2 as XCLK.
...
NODE
  ...
  echo "ready: generated CoreS3 camera sdkconfig at $combined_sdkconfig_dir"
